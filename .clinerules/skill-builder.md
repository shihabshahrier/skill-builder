...
**Phase A (--audit)**: read SKILL.md → run quality checklist → report failures/warnings/passed → if --fix output corrected SKILL.md

**Rules**:
- `name`: `^[a-z0-9]+(-[a-z0-9]+)*$`, max 64 chars; `description`: max 1024 chars; SKILL.md: max 5000 tokens
- Domain facts in references: WebSearch official docs first — never guess
